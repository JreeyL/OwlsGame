        ...
        stage('Deploy to EC2 APP') {
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: SSH_CREDENTIALS_ID, keyFileVariable: 'KEYFILE', usernameVariable: 'USERNAME')]) {
                    ...
                    ssh -i %KEYFILE% -o StrictHostKeyChecking=no %USERNAME%@${APP_HOST} "docker pull ${DOCKER_IMAGE_NAME}:latest && docker stop owlsgame-app owlsgame-db || true && docker rm owlsgame-app owlsgame-db || true && docker run -d --name owlsgame-app -p 8080:8080 -e SPRING_PROFILES_ACTIVE=docker ${DOCKER_IMAGE_NAME}:latest"
                    """
                }
            }
        }
